...
namespace App\Http\Controllers\ISI_6595\Entry\PumpTestingRD;
...
class EntryPumpTestingRD_Flow_6595_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $allEntryValues = isi_6595_EntryPumpTest::where('fldver', 1)->pluck('fldsno', 'fldpno');
            // dd($allEntryValues);
            $pType = '';
            ...
            if (count($request->query()) > 0) {
                ...
                if (sizeof($entries) == 0) return redirect()->route('6595_entryPumpTestRDFlow')->with('status', 'Data not exist!');
                return view('6595.entry.pumpTestingRD.flowmetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            return view('6595.entry.pumpTestingRD.flowmetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
            ...
            if ($request->isMethod('post') && $request->_token) {
                ...
                for ($i = 0; $i < $dataLength; $i++) {
                    ...
                    if (round($request->dis[$i], 1) == "0" || $request->dis[$i] == "-") {
                        ...
                        $Ad = $request->dis[$i];
                        $pumpd = isi_6595_EntryPumpTest::select('fldvol')->where('fldpno', '=', $request->pumpNo)->where('fldsno', '=', $pumpCheck->fldsno)->first();

                        if ($pumpd->fldvol != null) $time = $pumpd->fldvol / $Ad;
                        else $time = 1 / $Ad;
                    }
                ...
                if ($connection == null) $insert_connection = new isi_6595_Connection();
                ...
                return redirect()->route('6595_entryPumpTestRDFlow')->with('status', 'Record saved successfully!');
            } else return redirect()->route('6595_entryPumpTestRDFlow')->with('status', 'Invalid Request!');
        } catch (Exception $ex) {
            // dd($ex);
            return redirect()->back()->with('status', 'Internal error. Try again!' . $ex->__toString());
        }
    ...
    public function view_report_page(Request $request, $pumpNo, $pumpName)
    {
        try {
            ...
            if ($pumpDetails == null) return redirect()->route('6595_entryPumpTestRDFlow')->with('status', 'Pump not exist!');
            $entryDetails = isi_6595_EntryPumpTest::where('fldsno', '=', $pumpNo)->where('fldpno', '=', $pumpName)->get();
            if (sizeof($entryDetails) == 0) return redirect()->route('6595_entryPumpTestRDFlow')->with('status', 'Pump data not exist!');
            ...
            return view('6595.entry.pumpTestingRD.report_flow', compact('pumpData', 'tableData'));
        } catch (Exception $ex) {
    ...
    public function create_pdf()
    {
        ...
        $pdf = PDF::loadView('6595.entry.pumpTestingRD.report_flow', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('6595_entryPumpTestRDFlow')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
