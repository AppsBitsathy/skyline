...
class EntryPumpTestingISI_Flow_6595_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                if (sizeof($entries) == 0) return redirect()->route('6595_entryPumpTestISIFlow')->with('status', 'Data not exist!');
                return view('6595.entry.pumpTestingISI.flowmetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            return view('6595.entry.pumpTestingISI.flowmetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
        ...
        try {
            if ($request->isMethod('post') && $request->_token) {
                ...
                for ($i = 0; $i < $dataLength; $i++) {
                    ...
                    // return redirect()->route('6595_entryPumpTestISIFlow')->with('status', json_encode($y1));
                ...
                return redirect()->route('6595_entryPumpTestISIFlow')->with('status', 'Record saved successfully!');
            } else return redirect()->route('6595_entryPumpTestISIFlow')->with('status', 'Invalid Request!');
        } catch (Exception $ex) {
            // dd($ex);
            return redirect()->back()->with('status', 'Internal error. Try again!' . $ex->__toString());
        }
    ...
    public function view_report_page(Request $request, $pumpNo, $pumpName)
    {
        try {
            ...
            if ($pumpDetails == null) return redirect()->route('6595_entryPumpTestISIFlow')->with('status', 'Pump not exist!');
            $entryDetails = isi_6595_EntryPumpTest::where('fldsno', '=', $pumpNo)->where('fldpno', '=', $pumpName)->get();
            if (sizeof($entryDetails) == 0) return redirect()->route('6595_entryPumpTestISIFlow')->with('status', 'Pump data not exist!');
            ...
            return view('6595.entry.pumpTestingISI.report_flow', compact('pumpData', 'tableData'));
        } catch (Exception $ex) {
    ...
    public function create_pdf()
    {
        ...
        $pdf = PDF::loadView('6595.entry.pumpTestingISI.report_flow', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('6595_entryPumpTestISIFlow')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
