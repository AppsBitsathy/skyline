...
namespace App\Http\Controllers\ISI_8034\Entry\RoutineTest;

use App\Http\Controllers\Controller;
use App\Models\ISI_8034\Entry\PumpTest\isi_8034_EntryPumpTest;
use App\Models\ISI_8034\Entry\RoutineTesting\isi_8034_EntryRoutineTest;
use App\Models\ISI_8034\Entry\RoutineTesting\isi_8034_EntryRoutineTestReport;
use App\Models\ISI_8034\Master\isi_8034_MasterPumpTypes;
use Exception;
use Illuminate\Http\Request;
use Barryvdh\DomPDF\Facade as PDF;

class EntryRoutineTesting_8034_Controller extends Controller
{
    ...
    public function index(Request $request, $radioType = null, $typeValue = null)
    {
        try {
            ...
            $pumpNoDD = isi_8034_EntryRoutineTest::select('id', 'fldpno')->get();
            ...
            if ($radioType != null) {
                if ($typeValue != null) {
                    if ($radioType == 'pumpnowise') {
                        $tableData = isi_8034_EntryRoutineTest::join('isi_8034__master_pump_types', 'isi_8034__entry_routine_tests.fldptype', '=', 'isi_8034__master_pump_types.fldptype')
                            ->where('isi_8034__entry_routine_tests.id', '=', $typeValue)->get(['isi_8034__entry_routine_tests.*', 'isi_8034__master_pump_types.fldptype']);
                        $type = 'pnowise';
                        return view('8034.entry.routineTesting.routineTesting', compact('pumpDD', 'pumpNoDD', 'tableData', 'type'));
                    } else if ($radioType == 'pumptypewise') {
                        $check = isi_8034_EntryRoutineTest::select('*')->where('isi_8034__entry_routine_tests.fldptype', '=', $typeValue)->get();
                        if (count($check) != 0) {
                            $tableData = isi_8034_EntryRoutineTest::join('isi_8034__master_pump_types', 'isi_8034__entry_routine_tests.fldptype', '=', 'isi_8034__master_pump_types.fldptype')
                                ->where('isi_8034__master_pump_types.fldptype', '=', $typeValue)->get(['isi_8034__entry_routine_tests.*', 'isi_8034__master_pump_types.fldptype']);
                            $type = 'ptypewise';
                            return view('8034.entry.routineTesting.routineTesting', compact('pumpDD', 'pumpNoDD', 'tableData', 'type'));
                        } else {
            ...
            $tableData = isi_8034_EntryRoutineTest::join('isi_8034__master_pump_types', 'isi_8034__entry_routine_tests.fldptype', '=', 'isi_8034__master_pump_types.fldptype')
                ->get(['isi_8034__entry_routine_tests.*', 'isi_8034__master_pump_types.fldptype']);
    ...
    public function store(Request $request)
    {
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                $pump = isi_8034_MasterPumpTypes::where('fldptype', '=', $request->pumpType)->first();

                if ($pump == null) return redirect()->route('8034_entryRoutineTesting')->with('status', 'Pump not exist');

                $rs1 = isi_8034_EntryPumpTest::where('fldsno', '=', $pump->fldsno)->get();
                ...
                if (sizeof($rs1) > 0) {
                    ...
                    if ($request->input('id')) {
                        $routineTest = isi_8034_EntryRoutineTest::findOrFail($request->input('id'));
                        $msg = 'Record updated successfully!';
                    } else {
                        $routineTest = new isi_8034_EntryRoutineTest();
    ...
    public function view_report_page(Request $request)
    {
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                $reportData = isi_8034_EntryRoutineTest::where('id', '=', $req->reportPnoId)->get();

                $pump = isi_8034_MasterPumpTypes::where('fldptype', '=', $reportData[0]->fldptype)->first();

                $this->store_report($req, $pump, $reportData);

                $tableData = isi_8034_EntryRoutineTestReport::all();

                return view('8034.entry.routineTesting.report', compact('tableData'));
            } else {
    ...
    public function view_custom_report_page(Request $request)
    {
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                $reportData = isi_8034_EntryRoutineTest::where('fldptype', '=', $req->pumpId)
    ...
    public function create_pdf_report(Request $request)
    {
        try {
            $tableData = isi_8034_EntryRoutineTestReport::all();
            $pdf = PDF::loadView('8034.entry.routineTesting.report', compact('tableData'))->setPaper('a3', 'landscape');
            return $pdf->download();
        } catch (Exception $ex) {
    ...
    public function store_report($req, $pump, $tableData)
    {
        try {
            //
            $report = new isi_8034_EntryRoutineTestReport();
            $report->truncate();
            ...
            foreach ($tableData as $data) {
                ...
                $vol = isi_8034_EntryPumpTest::where('fldsno', '=', $pump->fldsno)->first();
    ...
    public function delete(Request $request)
    {
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                isi_8034_EntryRoutineTest::where('id', '=', $request->input('deletePumpIpNoId'))->delete();
                return redirect()->route('8034_entryRoutineTesting')->with('status', 'Record Deleted!');
