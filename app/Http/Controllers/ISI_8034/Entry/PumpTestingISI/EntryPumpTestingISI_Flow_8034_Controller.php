...
class EntryPumpTestingISI_Flow_8034_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                if (sizeof($entries) == 0) return redirect()->route('8034_entryPumpTestISIFlow')->with('status', 'Data not exist!');
                return view('8034.entry.pumpTestingISI.flowmetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            return view('8034.entry.pumpTestingISI.flowmetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
            ...
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                for ($i = 0; $i < $dataLength; $i++) {
                    if ($request->dis[$i] == "0" || $request->dis[$i] == "-" || $request->dis[$i] == 0) {
                        ...
                        $Ad = $request->dis[$i];
                        $pumpd = isi_8034_EntryPumpTest::select('fldvol')->where('fldsno', '=', $pumpCheck->fldsno)->where('fldpno', '=', $request->pumpNo)->first();
    ...
    public function view_report_page(Request $request, $pumpNo = null, $pumpName = null)
    {
        try {
            if ($pumpNo != null) {
                ...
                if ($pumpDetails == null) return redirect()->route('8034_entryPumpTestISIFlow')->with('status', 'Pump not exist!');
                $entryDetails = isi_8034_EntryPumpTest::where('fldsno', '=', $pumpNo)->where('fldpno', '=', $pumpName)->get();
                if (sizeof($entryDetails) == 0) return redirect()->route('8034_entryPumpTestISIFlow')->with('status', 'Pump data not exist!');
                ...
                return view('8034.entry.pumpTestingISI.report_flow', compact('pumpData', 'tableData'));
            } else return redirect()->route('8034_entryPumpTestISIFlow')->with('status', 'Pump not exist!');
        } catch (Exception $ex) {
            return redirect()->back()->with('status', 'Internal Error!' . $ex->__toString());
        }
    }
    ...
    public function create_pdf()
    {
        ...
        $pdf = PDF::loadView('8034.entry.pumpTestingISI.report_flow', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('8034_entryPumpTestISIFlow')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
