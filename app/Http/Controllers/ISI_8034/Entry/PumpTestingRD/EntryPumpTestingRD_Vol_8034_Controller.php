...
class EntryPumpTestingRD_Vol_8034_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                return view('8034.entry.pumpTestingRD.volumetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            return view('8034.entry.pumpTestingRD.volumetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
            ...
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                for ($i = 0; $i < $dataLength; $i++) {
                    ...
                    if (round($request->time[$i], 1) == "0" || $request->time[$i] == "-") {
                        ...
                        $Ad = ($request->collTankVol) / $request->time[$i];
                    }
    ...
    public function view_report_page(Request $request, $pumpNo = null, $pumpName = null)
    {
        try {
            ...
            if ($pumpDetails == null) return redirect()->route('8034_entryPumpTestRDVol')->with('status', 'Pump not exist!');
            $entryDetails = isi_8034_EntryPumpTest::where('fldsno', '=', $pumpNo)->where('fldpno', '=', $pumpName)->get();
            if (sizeof($entryDetails) == 0) return redirect()->route('8034_entryPumpTestRDVol')->with('status', 'Pump data not exist!');
            ...
            return view('8034.entry.pumpTestingRD.report_vol', compact('pumpData', 'tableData'));
        } catch (Exception $ex) {
            return redirect()->back()->with('status', 'Internal Error!' . $ex->__toString());
        }
    }
    ...
    public function create_pdf()
    {
        ...
        $pdf = PDF::loadView('8034.entry.pumpTestingRD.report_vol', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('8034_entryPumpTestRDVol')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
