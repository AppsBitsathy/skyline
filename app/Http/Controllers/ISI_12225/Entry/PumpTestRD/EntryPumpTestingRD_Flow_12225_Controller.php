...
namespace App\Http\Controllers\ISI_12225\Entry\PumpTestRD;
...
class EntryPumpTestingRD_Flow_12225_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                if (sizeof($entries) == 0) return redirect()->route('12225_entryPumpTestRDFlow')->with('status', 'Data not exist!');
                return view('12225.entry.pumpTestingRD.flowmetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            // return view('12225.entry.pumpTestingRD.volumetric', compact('allPumps'));
            return view('12225.entry.pumpTestingRD.flowmetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
        ...
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                for ($i = 0; $i < $dataLength; $i++) {
                    ...
                    $insert->fldrandd = true; // RandD = true
                    $insert->fld1_5 = $t1s;
                    $insert->fld2 = $t2d;
                    $insert->fld2m = $t2dm;
    ...
    public function view_report_page(Request $request, $pumpNo = null, $pumpName = null)
    {
        try {
            if ($pumpNo != null) {
                ...
                if ($pumpDetails == null) return redirect()->route('12225_entryPumpTestRDFlow')->with('status', 'Pump not exist!');
                $entryDetails = isi_12225_EntryPumpTest::where('fldsno', '=', $pumpNo)->where('fldpmno', '=', $pumpName)->get();
                if (sizeof($entryDetails) == 0) return redirect()->route('12225_entryPumpTestRDFlow')->with('status', 'Pump data not exist!');
                ...
                return view('12225.entry.pumpTestingRD.report_flow', compact('pumpData', 'tableData'));
                //
            } else return redirect()->route('12225_entryPumpTestRDVol')->with('status', 'Pump not exist!');
        } catch (Exception $ex) {
        }
    }
    ...
    public function create_pdf()
    {
        ...
        $pdf = PDF::loadView('12225.entry.pumpTestingRD.report_flow', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('12225_entryPumpTestRDFlow')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
