...
class EntryPumpTestingISI_Vol_8472_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                return view('8472.entry.pumpTestingISI.volumetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            // return view('8472.entry.pumpTestingISI.volumetric', compact('allPumps'));
            return view('8472.entry.pumpTestingISI.volumetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
    ...
    public function view_report_page(Request $request)
    {
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                if ($pumpDetails == null) return redirect()->route('8472_entryPumpTestISIVol')->with('status', 'Pump not exist!');
                $entryDetails = isi_8472_EntryPumpTest::where('fldsno', '=', $request->obssptimepumpNo)->where('fldpmno', '=', $request->obssptimepumpName)->get();
                if (sizeof($entryDetails) == 0) return redirect()->route('8472_entryPumpTestISIVol')->with('status', 'Pump data not exist!');
                ...
                return view('8472.entry.pumpTestingISI.report_vol', compact('pumpData', 'tableData'));
            } else return redirect()->back()->with('status', 'Invalid request');
        } catch (Exception $ex) {
            return redirect()->back()->with('status', 'Internal Error!' . $ex->__toString());
        }
    ...
    public function create_pdf()
    {
        ...
        $pdf = PDF::loadView('8472.entry.pumpTestingISI.report_vol', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('8472_entryPumpTestISIVol')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
