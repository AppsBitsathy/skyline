...
namespace App\Http\Controllers\ISI_8472\Entry\PumpTestingRD;
...
class EntryPumpTestingRD_Vol_8472_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                if (sizeof($entries) == 0) return redirect()->route('8472_entryPumpTestISIVol')->with('status', 'Data not exist!');
                return view('8472.entry.pumpTestingRD.volumetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            return view('8472.entry.pumpTestingRD.volumetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
        ...
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                for ($i = 0; $i < $dataLength; $i++) {
                    ...
                    if (round($request->time[$i], 1) == "0" || $request->time[$i] == "-") {
                        $Ad = 0;
                    } else {
                        $Ad = ($request->collTankVol) / $request->time[$i] * 3600;
                    }
    ...
    public function view_report_page(Request $request)
    {
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                if ($pumpDetails == null) return redirect()->route('8472_entryPumpTestRDVol')->with('status', 'Pump not exist!');
                $entryDetails = isi_8472_EntryPumpTest::where('fldsno', '=', $request->obssptimepumpNo)->where('fldpmno', '=', $request->obssptimepumpName)->get();
                if (sizeof($entryDetails) == 0) return redirect()->route('8472_entryPumpTestRDVol')->with('status', 'Pump data not exist!');
                ...
                return view('8472.entry.pumpTestingRD.report_vol', compact('pumpData', 'tableData'));
            } else return redirect()->back()->with('status', 'Invalid request');
        } catch (Exception $ex) {
            return redirect()->back()->with('status', 'Internal Error!' . $ex->__toString());
        }
    ...
    public function create_pdf()
    {
        ...
        $pdf = PDF::loadView('8472.entry.pumpTestingRD.report_vol', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('8472_entryPumpTestRDVol')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
