...
class EntryPumpTestingRD_Flow_8472_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                if (sizeof($entries) == 0) return redirect()->route('8472_entryPumpTestRDFlow')->with('status', 'Data not exist!');
                return view('8472.entry.pumpTestingRD.flowmetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            return view('8472.entry.pumpTestingRD.flowmetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
    ...
    public function view_report_page(Request $request)
    {
        try {
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                if ($pumpDetails == null) return redirect()->route('8472_entryPumpTestRDFlow')->with('status', 'Pump not exist!');
                $entryDetails = isi_8472_EntryPumpTest::where('fldsno', '=', $request->obssptimepumpNo)->where('fldpmno', '=', $request->obssptimepumpName)->get();
                if (sizeof($entryDetails) == 0) return redirect()->route('8472_entryPumpTestRDFlow')->with('status', 'Pump data not exist!');
                ...
                return view('8472.entry.pumpTestingRD.report_flow', compact('pumpData', 'tableData'));
            } else return redirect()->back()->with('status', 'Invalid request');
        } catch (Exception $ex) {
            return redirect()->back()->with('status', 'Internal Error!' . $ex->__toString());
        }
    ...
    public function create_pdf(Request $request)
    {
        ...
        $pdf = PDF::loadView('8472.entry.pumpTestingRD.report_flow', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('8472_entryPumpTestRDFlow')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
