...
class EntryPumpTestingISI_Vol_14220_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                if (sizeof($entries) == 0) return redirect()->route('14220_entryPumpTestISIVol')->with('status', 'Data not exist!');
                return view('14220.entry.pumpTestingISI.volumetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            // return view('14220.entry.pumpTestingISI.volumetric', compact('allPumps'));
            return view('14220.entry.pumpTestingISI.volumetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
            dd($ex);
        }
    }
    ...
    public function view_report_page(Request $request, $pumpNo = null, $pumpName = null)
    {
        try {
            if ($pumpNo != null) {
                ...
                if ($pumpDetails == null) return redirect()->route('14220_entryPumpTestISIVol')->with('status', 'Pump not exist!');
                $entryDetails = isi_14220_EntryPumpTest::where('fldsno', $pumpNo)->where('fldpno', $pumpName)->get();
                if (sizeof($entryDetails) == 0) return redirect()->route('14220_entryPumpTestISIVol')->with('status', 'Pump data not exist!');
                ...
                return view('14220.entry.pumpTestingISI.report_vol', compact('pumpData', 'tableData'));
            } else return redirect()->route('14220_entryPumpTestISIVol')->with('status', 'Pump not exist!');
        } catch (Exception $ex) {
            dd($ex);
        }
    }
    ...
    public function create_pdf()
    {
        try {
            ...
            $pdf = PDF::loadView('14220.entry.pumpTestingISI.report_vol', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('14220_entryPumpTestISIVol')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
            dd($ex);
        }
    }
