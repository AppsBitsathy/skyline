...
namespace App\Http\Controllers\ISI_14220\Entry\PumpTestingRD;
...
class EntryPumpTestingRD_Flow_14220_Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (count($request->query()) > 0) {
                ...
                if (sizeof($entries) == 0) return redirect()->route('14220_entryPumpTestRDFlow')->with('status', 'Data not exist!');
                return view('14220.entry.pumpTestingRD.flowmetric', compact('allPumps', 'entries', 'allEntryValues', 'pType'));
            }
            return view('14220.entry.pumpTestingRD.flowmetric', compact('allPumps', 'allEntryValues'));
        } catch (Exception $ex) {
            ...
            if ($request->isMethod('post') && $request->input('_token')) {
                ...
                for ($i = 0; $i < $dataLength; $i++) {
                    ...
                    if (round($request->dis[$i], 1) == "0" || $request->dis[$i] == "-") {
                        ...
                        $Ad = $request->dis[$i];
                        $pumpd = isi_14220_EntryPumpTest::select('fldvol')->where('fldpno', '=', $request->pumpNo)->where('fldsno', '=', $pumpCheck->fldsno)->first();

                        if ($pumpd->fldvol != null)
                            $time = $pumpd->fldvol / $Ad;
                        else
                            $time = 1 / $Ad;
                    }
    ...
    public function view_report_page(Request $request, $pumpNo = null, $pumpName = null)
    {
        try {
            ...
            if ($pumpDetails == null) return redirect()->route('14220_entryPumpTestRDFlow')->with('status', 'Pump not exist!');
            $entryDetails = isi_14220_EntryPumpTest::where('fldsno', '=', $pumpNo)->where('fldpno', '=', $pumpName)->get();
            if (sizeof($entryDetails) == 0) return redirect()->route('14220_entryPumpTestRDFlow')->with('status', 'Pump data not exist!');
            ...
            return view('14220.entry.pumpTestingRD.report_flow', compact('pumpData', 'tableData'));
        } catch (Exception $ex) {
            return redirect()->back()->with('status', 'Internal Error!' . $ex->__toString());
        }
    }
    ...
    public function create_pdf(Request $request)
    {
        ...
        $pdf = PDF::loadView('14220.entry.pumpTestingRD.report_flow', compact('pumpData', 'tableData'))->setPaper('a3', 'landscape');
    ...
    public function delete(Request $request)
    {
        try {
            ...
            return redirect()->route('14220_entryPumpTestRDFlow')->with('status', 'Record Deleted!');
        } catch (Exception $ex) {
