...
@section('content')
            ...
            <div id="entry" class="col m12">
                <div class="card">
                    <div class="card-body pt-5 pb-2">
                        <form method="POST" action="{{ route('6595_entryMotorTestingEntry') }}">
                            @csrf
                            <div class="row">
                    ...
                    <div class="card-body">
                        <div class="row mb-4">
                            <div class="col m4 center-align">
                                <form action="{{ route('6595_entryMotorTestingDelete') }}" method="post">
                                    @csrf
                            ...
                            <div class="col m4 center-align">
                                <form action="{{ route('6595_entryMotorTestingViewReport') }}" method="post"
                                    target="blank">
                                    @csrf
    ...
    <div id="reportCustomDate" class="modal">
        <form method="post" action="{{ route('6595_entryMotorTestingViewCustomReport') }}" id="frmCustomReportDate"
            target="blank">
            @csrf
            <div class="modal-content">
...
@section('custom-script')
    <script>
        ...
        $('#listInpassWise').change(function(e) {
            ...
            window.location.href = "{{ URL::to('6595/entry/motor_testing') }}" + "/motorinpasswise/" +
                selectedInpass;
        });

        $('#listMotorWise').change(function(e) {
            let selectedMotor = e.target.value;
            window.location.href = "{{ URL::to('6595/entry/motor_testing') }}" + "/motortypewise/" +
                selectedMotor;
        });

        $('#motorlistall').change(function(e) {
            window.location.href = "{{ URL::to('6595/entry/motor_testing') }}";
        })
