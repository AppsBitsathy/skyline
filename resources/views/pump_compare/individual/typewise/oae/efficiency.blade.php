...
<title>Pump Comparison - Individual Curve - Typewise - Efficiency</title>
@php
$rId = 4212;
$ssrId = 421;
$srId = 42;
    ...
    <script>
        ...
        $('#ddOpenPumpType').change((e) => {
            ...
            @foreach ($volumetricValues as $v)
                ...
                html += '<option value="{{ $v->fldPno }}">{{ $v->fldPno }}</option>';
                }
            @endforeach

            @foreach ($flowmetricValues as $f)
                if (selectedPump == {{ $f->fldSno }}) {
                html += '<option value="{{ $f->fldPno }}">{{ $f->fldPno }}</option>';
                }
            @endforeach
