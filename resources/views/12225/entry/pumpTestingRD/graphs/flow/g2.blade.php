    ...
    <script>
        ...
        function defaultGraph(visibleX1, visibleX2, visibleX3, selectedXAxis) {
            ...
            for (let i = 0; i < trace1x.length; i++) {
                for (let j = trace1x[i]; j <= trace1x[i + 1]; j = j + 0.050) {
                    if (j > 0) {
                        var l1, l2, l3, y1;
            ...
            for (let i = 0; i < trace2x.length; i++) {
                for (let j = trace2x[i]; j <= trace2x[i + 1]; j = j + 0.050) {
                    if (j > 0) {
            ...
            for (let i = 0; i < trace3x.length; i++) {
                for (let j = trace3x[i]; j <= trace3x[i + 1]; j = j + 0.050) {
                    if (j > 0) {
                        var l1, l2, l3, y1, t1, t2;
                        ...
                        l1 = ((j - trace3x[i + 1]) * (j - altXPlus2)) / ((trace3x[i] - trace3x[i + 1]) * (trace3x[i] -
                            altXPlus2));
                        t1 = (j - trace3x[i]) * (j - altXPlus2);
                        t2 = (trace3x[i + 1] - altXPlus2) * (trace3x[i + 1] - trace3x[i]);
                        l2 = t1 / t2;
                        // l2 = ((j - trace3x[i]) * (j - altXPlus2)) / ((trace3x[i + 1] - altXPlus2) * (trace3x[i + 1] - trace3x[i]));
                        l3 = ((j - trace3x[i]) * (j - trace3x[i + 1])) / ((altXPlus2 - trace3x[i]) * (altXPlus2 - trace3x[
                            i + 1]));
            ...
            sample4x = [];
            sample4y = [];
            for (let i = 0; i < trace4x.length; i++) {
                for (let j = trace4x[i]; j <= trace4x[i + 1]; j = j + 0.050) {
                    if (j > 0) {
                        var l1, l2, l3, y1;
                        var altXPlus2 = trace4x[i + 2];
                        var altYPlus2 = trace4y[i + 2];
                        if (trace4x[i + 2] == undefined) {
                            altXPlus2 = 0;
                            altYPlus2 = 0;
                        }
                        l1 = ((j - trace4x[i + 1]) * (j - altXPlus2)) / ((trace4x[i] - trace4x[i + 1]) * (trace4x[i] -
                            altXPlus2));
                        l2 = ((j - trace4x[i]) * (j - altXPlus2)) / ((trace4x[i + 1] - altXPlus2) * (trace4x[i + 1] -
                            trace4x[i]));
                        l3 = ((j - trace4x[i]) * (j - trace4x[i + 1])) / ((altXPlus2 - trace4x[i]) * (altXPlus2 - trace4x[
                            i + 1]));
                        y1 = l1 * trace4y[i] + l2 * trace4y[i + 1] + l3 * altYPlus2;
                        sample4x.push(j);
                        sample4y.push(y1);

                    } else {
                        sample4x.push(j);
                        sample4y.push(trace4y[i]);
                    }
                }
            }
        ...
        function downloadToExcel() {
            ...
            window.location.href = "{{ URL::to('12225/entry/pump_testing_rd/graphs/flow/g2/excel') }}" + "?" +
                parameters[1];
        }
