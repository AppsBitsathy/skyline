...
@section('custom-script')
    ...
    <script>
        ...
        function defaultGraph(visibleX1, visibleX2, visibleX3, selectedXAxis) {
            ...
            rec = trace4x.length + 1;
            for (let i = 0; i <= (rec - 2); i++) {
                var altXPlus1 = trace4x[i + 1];
                var altYPlus1 = trace4y[i + 1];
                if (trace4x[i + 1] == undefined) {
                    altXPlus1 = 0;
                    altYPlus1 = 0;
                }
                h[i] = altXPlus1 - trace4x[i];
                d[i] = (altYPlus1 - trace4y[i]) / h[i];
                if (i > 0) {
                    u[i] = parseFloat(6 * (d[i] - d[i - 1])).toFixed(4);
                }
                if (i == 1) {
            ...
            for (let k = 0; k <= rec - 2; k++) {
                ...
                sk3 = (tempUse2 - tempUse1) / (6 * h[k]);
                let jj;
                for (let j = i; j <= trace4x[k + 1]; j = j + 0.050) {
                    w = j - trace4x[k];
                    yp = ((((sk3 * w) + sk2) * w + sk1) * w) + trace4y[k];
                    sample4x.push(j);
                    sample4y.push(yp);
                    jj = j;
                }
        ...
        function downloadToExcel() {
            ...
            window.location.href = "{{ URL::to('12225/entry/pump_testing_rd/graphs/flow/g3/excel') }}" + "?" +
                parameters[1];
        }
