...
@section('custom-script')
    ...
    <script>
        ...
        @foreach ($volumetricsValues as $vol)
            xx1[tempj] = {{ $vol['fldrdis'] }};
            yy1[tempj] = {{ $vol['fldrthead'] }};
            xx2[tempj] = {{ $vol['fldrdis'] }};
            yy2[tempj] = {{ $vol['fldrdlwl'] }};
            xx3[tempj] = {{ $vol['fldrdis'] }};
            yy3[tempj] = {{ $vol['fldipow'] }};
            xx4[tempj] = {{ $vol['fldrdis'] }};
            yy4[tempj] = {{ $vol['fldcurr'] }};
            tempj = tempj + 1;
        @endforeach
        ...
        function defaultGraph(visibleX1, visibleX2, visibleX3, selectedXAxis) {
            ...
            let sv = {{ $volumetricsValuesASC[0]['fldrdis'] / 16.99999 }};
            ...
            sv = {{ $volumetricsValuesASC[0]['fldrdis'] / 16.99999 }};
            ...
            @for ($i = 0; $i < count($volumetricsValues); $i++)
                trace1x[{{ $i }}] = {{ $volumetricsValues[$i]['fldrdis'] }};
                trace1y[{{ $i }}] = yy1[{{ $i }}];
                trace2x[{{ $i }}] = {{ $volumetricsValues[$i]['fldrdis'] }};
                trace2y[{{ $i }}] = yy2[{{ $i }}];
                trace3x[{{ $i }}] = {{ $volumetricsValues[$i]['fldrdis'] }};
                trace3y[{{ $i }}] = yy3[{{ $i }}];
                trace4x[{{ $i }}] = {{ $volumetricsValues[$i]['fldrdis'] }};
                trace4y[{{ $i }}] = yy4[{{ $i }}];
            @endfor
            // }
            // else {
        ...
        function downloadToExcel() {
            ...
            window.location.href = "{{ URL::to('12225/entry/pump_testing_rd/graphs/vol/g9/excel') }}" + "?" +
                parameters[1];
        }
