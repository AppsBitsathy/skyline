...
@section('custom-script')
    <script>
        ...
        for (let i = 0; i < trace1x.length; i++) {
            for (let j = trace1x[i]; j <= trace1x[i + 1]; j = j + 0.003) {
                if (j > 0) {
                    var l1, l2, l3, y1;
        ...
        for (let i = 0; i < trace2x.length; i++) {
            for (let j = trace2x[i]; j <= trace2x[i + 1]; j = j + 0.003) {
                if (j > 0) {
        ...
        for (let i = 0; i < trace3x.length; i++) {
            for (let j = trace3x[i]; j <= trace3x[i + 1]; j = j + 0.003) {
                if (j > 0) {
                    var l1, l2, l3, y1, t1, t2;
        ...
        for (let i = 0; i < trace4x.length; i++) {
            for (let j = trace4x[i]; j <= trace4x[i + 1]; j = j + 0.003) {
                if (j > 0) {
                    var l1, l2, l3, y1;
        ...
        function obsValues(discharge = {{ $pumpValues['flddis'] }}, tHead = {{ $pumpValues['fldthead'] }}, dlwl1 =
            ...
            // Plotly.addTraces('myDiv',qvsdlwlTrace);
            // Plotly.addTraces('myDiv',qvsthTrace);
            // Plotly.addTraces('myDiv',qvsipTrace);
            // Plotly.addTraces('myDiv',dpvsipTrace);
            // Plotly.addTraces('myDiv',traceReplaced7);
            // Plotly.addTraces('myDiv',traceReplaced8);
            // Plotly.addTraces('myDiv',trace3SubTrace);
            // Plotly.addTraces('myDiv',trace3SubTrace2);
