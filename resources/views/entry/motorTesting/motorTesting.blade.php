...
@section('content')
            ...
            <div id="entry" class="col m12">
                <div class="card">
                    <div class="card-body pt-5 pb-2">
                        <form method="POST" action="{{ route('entryMotorTestingEntry') }}">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col m4 pl-5 pr-5">
                                    <span>Motor No.</span>
                                    <input class="input-field" name="motorNo" placeholder="Motor No." required
                                        autocomplete="on">
                                </div>
                                <div class="col m4 pl-5 pr-5">
                                ...
                                <div class="col m12 pl-5 pr-5">
                                    <h6><u><b>Temperature Rise Test at 240V</b></u></h6>
                                </div>
                                <div class="col m4 pl-5 pr-5">
                                    <span>R2 (ohm)</span>
                    ...
                    <div class="card-body">
                        <div class="row mb-4">
                            <div class="col m4 center-align">
                                <form action="{{ route('entryMotorTestingDelete') }}" method="post">
                                    @csrf
                            ...
                            <div class="col m4 center-align">
                                <form action="{{ route('entryMotorTestingViewReport') }}" method="post" target="blank">
                                    @csrf
                                        ...
                                        <select name="listInpassWise" id="listInpassWise">
                                            ...
                                            @foreach ($allMotorsInpassDD as $inpass)
                                                <option value="{{ $inpass->id }}">
                                                    {{ $inpass->fldinpass . ' - ' . $inpass->fldMotorType }}</option>
                                            @endforeach
                                        </select>
                        ...
                        <tbody>
                            ...
                            @foreach ($allMotorTestEntry as $motorTestEntry)
                                <tr>
                                    ...
                                    <td>{{ $motorTestEntry->fldMotorType }}</td>
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('entryMotorTestingEntry') }}">
                ...
                <div class="row">
                    ...
                    <div class="col m4 pl-5 pr-5">
                        <span>Motor No.</span>
                        <input class="input-field" name="motorNo" id="motorNo" readonly>
                    </div>
                    <div class="col m4 pl-5 pr-5">
    ...
    <div id="reportCustomDate" class="modal">
        <form method="post" action="{{ route('entryMotorTestingViewCustomReport') }}" id="frmCustomReportDate"
            target="blank">
            @csrf
            <div class="modal-content">
    ...
    <script>
        $(document).ready(function() {
            ...
            if ('{{ $type }}' == 'mtype') {
                ...
                $('#customReportMotorid').val(
                    '{{ count($allMotorTestEntry) > 0 ? $allMotorTestEntry[0]->fldmtype : 0 }}');
            } else if ('{{ $type }}' == 'inpass') {
                $('#btnReport').removeAttr('disabled');
                $('#btnDelete').removeAttr('disabled');
                $('#deleteMotorIpNoId').val(
                    '{{ count($allMotorTestEntry) > 0 ? $allMotorTestEntry[0]->id : 0 }}');
        ...
        function edit(motor) {
            ...
            $('#trtCasePrtb').val(motor.fldcptb);
            $('#date').val(motor.flddate.split(' ')[0]);
        }

        $('#listInpassWise').change(function(e) {
            let selectedInpass = e.target.value;
            window.location.href = "{{ URL::to('9079/entry/motor_testing') }}" + "/motorinpasswise/" +
                selectedInpass;
        });

        $('#listMotorWise').change(function(e) {
            let selectedMotor = e.target.value;
            window.location.href = "{{ URL::to('9079/entry/motor_testing') }}" + "/motortypewise/" +
                selectedMotor;
        });

        $('#motorlistall').change(function(e) {
            window.location.href = "{{ URL::to('9079/entry/motor_testing') }}";
        })
