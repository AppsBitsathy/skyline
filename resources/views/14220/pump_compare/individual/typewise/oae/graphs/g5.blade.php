...
@section('custom-script')
    ...
    if (altXPlus1 == undefined) { altXPlus1 = 0; }
    if (altYPlus1 == undefined) { altYPlus1 = 0; }
    h[i] = altXPlus1 -
        xy[i][0];
...
for (let k = 1; k <= (n - 1); k++) { // Start Pivot 
    var large, temp11, p; p = k; large = Math.abs(a[k][k]); for (let vi = (k + 1); vi <= n; vi++)
    {
    ...
    if (p != k) {
        for (let vj = k; vj <= n; vj++) { temp11 = a[p][vj];
            a[p][vj] = a[k][vj];
            a[k][vj] = temp11; }
        temp11 = b[p];
        b[p] = b[k];
...
} //End of TotalHead Curve Formula // Efficiency Formula
sample2x = []; sample2y = []; xy = []; k; jj;
h = [];
d = [];
u = [];
a = [];
...
} // End of CurrentFormula // Total Head 
trace1 = {
    x: sample1x, y: sample1y,
...
@endforeach
        ...
        function callG1() {
            ...
            window.location.href = "{{ URL::to('14220/pump_comparison/individual_curve/typewise/oae/graphs/g1') }}" + "?" + parameters[1];
        }

        function callG2() {
            let parameters = window.location.href.split('?');
            window.location.href = "{{ URL::to('14220/pump_comparison/individual_curve/typewise/oae/graphs/g2') }}" + "?" + parameters[1];
        }

        function callG3() {
            let parameters = window.location.href.split('?');
            window.location.href = "{{ URL::to('14220/pump_comparison/individual_curve/typewise/oae/graphs/g3') }}" + "?" + parameters[1];
        }

        function callG4() {
            let parameters = window.location.href.split('?');
            window.location.href = "{{ URL::to('14220/pump_comparison/individual_curve/typewise/oae/graphs/g4') }}" + "?" + parameters[1];
        }

        function callG5() {
            let parameters = window.location.href.split('?');
            window.location.href = "{{ URL::to('14220/pump_comparison/individual_curve/typewise/oae/graphs/g5') }}" + "?" + parameters[1];
        }

        function callG6() {
            let parameters = window.location.href.split('?');
            window.location.href = "{{ URL::to('14220/pump_comparison/individual_curve/typewise/oae/graphs/g6') }}" + "?" + parameters[1];
        }
